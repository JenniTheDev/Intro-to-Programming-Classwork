...
int main(int argc, char** argv) {
    ...
    for(int year=0,dateYr=1626;year<=nYears;year++,dateYr++){
        cout<<setw(4)<<year<<setw(7)<<dateYr<<setw(12)<<setprecision(2)<<fv<<endl;
        fv*=(1+iRate);//Each year pay yourself some interest
        int ifv=fv*100;//shifts from dollars to pennies then truncate
        fv=ifv/100.0f;//Truncating to the nearest penny back to dollars
    }
